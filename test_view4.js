...
async function run() {
  const { data, error } = await supabase.from('datasets').select('preview_data').eq('slug', 'dsex-prices-historical');
  console.log("Preview Data:", JSON.stringify(data[0].preview_data, null, 2));
}
run();
