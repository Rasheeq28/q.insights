...
async function run() {
  const { data, error } = await supabase.from('dsex_prices').select('trading_code,sector,date,openp,high,low,closep,volume').limit(2);
  console.log("dsex_prices Data:", JSON.stringify(data, null, 2));
